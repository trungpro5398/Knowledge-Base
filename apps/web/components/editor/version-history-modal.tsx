...
import { usePageVersion, useVersionHistory } from "@/lib/api/hooks";
import { api } from "@/lib/api/client";
import { DiffViewer } from "./diff-viewer";
import { Skeleton } from "@/components/ui/skeleton";
import { toast } from "sonner";
import { useLocale } from "@/lib/i18n/locale-provider";
import type { PageVersion, PageVersionSummary } from "@/lib/api/types";
...
export function VersionHistoryModal({
  ...
  const expandedIndex = versions?.findIndex((version) => version.id === expandedId) ?? -1;
  const previousVersionId =
    !compareWithCurrent && expandedIndex >= 0
      ? versions?.[expandedIndex + 1]?.id
      : undefined;
  const { data: expandedVersion, isLoading: isLoadingExpandedVersion } = usePageVersion(pageId, expandedId);
  const { data: previousVersion, isLoading: isLoadingPreviousVersion } = usePageVersion(
    pageId,
    previousVersionId
  );

  const handleRestore = async (version: PageVersionSummary) => {
    try {
      const res = await api.get<{ data: PageVersion }>(
        `/api/pages/${pageId}/versions/${version.id}`
      );
      const fullVersion = res.data;
      const dateStr = new Date(fullVersion.created_at).toLocaleString();
      if (confirm(t("version.restoreConfirm", { date: dateStr }))) {
        onRestore(fullVersion.content_md || "");
                  ...
                  <div key={version.id} className="border rounded-lg overflow-hidden hover:border-primary/50 transition-colors">
                    ...
                    {isExpanded && (
                      <div id={`version-panel-${version.id}`} className="px-4 pb-4">
                        {isLoadingExpandedVersion || (!compareWithCurrent && isLoadingPreviousVersion) ? (
                          <Skeleton className="h-48 w-full" />
                        ) : (
                          <DiffViewer
                            oldText={compareWithCurrent ? currentContent : (previousVersion?.content_md || "")}
                            newText={expandedVersion?.content_md || ""}
